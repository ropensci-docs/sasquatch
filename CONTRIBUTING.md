...
- Fork the package and clone onto your computer. If you haven’t done
  ...
  `usethis::create_from_github("ropensci/sasquatch", fork = TRUE)`.

- Install all development dependencies with
  `devtools::install_dev_deps()`.

- Install Python (we recommend
  [`reticulate::install_python()`](https://rstudio.github.io/reticulate/reference/install_python.html))
  or Java if not already installed.

- Configure `SASPy` by using
  [`sasquatch::install_saspy()`](https://docs.ropensci.org/sasquatch/reference/install_saspy.md).
  If you do not have access to a SAS client, we recommend using SAS On
  Demand for Academics (SAS). Check out
  [`vignette('configuration')`](https://docs.ropensci.org/sasquatch/articles/configuration.md)
  for more information on how to set up `SASPy` with ODA.  
...
Please note that the sasquatch project is released with a [Contributor
Code of
Conduct](https://docs.ropensci.org/sasquatch/CODE_OF_CONDUCT.md). By
